...
export const TabContent: React.FC<TabContentProps> = ({ activeTab, activeSession, isGeneratingTTS, onGenerateTTS }) => {
  ...
  switch (activeTab) {
      ...
      return (
        <div>
          {!summary.tldr || summary.tldr === "{}" ? (
            <div style={{ color: 'var(--text-muted)', fontSize: '14px', fontStyle: 'italic', marginBottom: '24px' }}>
              Summary data is missing or improperly formatted. Try regenerating.
            </div>
          ) : (
            <div 
          ...
          <button
            ...
            disabled={isGeneratingTTS || !summary.tldr}
            style={{ display: 'flex', alignItems: 'center', gap: '7px', padding: '8px 16px', borderRadius: '8px', cursor: (isGeneratingTTS || !summary.tldr) ? 'not-allowed' : 'pointer', fontWeight: 600, fontSize: '13px', border: '1px solid var(--border)', background: 'transparent', color: 'var(--text-secondary)', transition: 'all 0.2s' }}
          >
      ...
      return (
        <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '10px' }}>
          {summary.keyConcepts && summary.keyConcepts.length > 0 ? (
            ...
            <div style={{ gridColumn: '1 / -1', color: 'var(--text-muted)', fontSize: '14px', textAlign: 'center', padding: '40px' }}>
    ...
    case 'detailed': {
      const hasContent = summary.detailedSummary && summary.detailedSummary.length > 0;
      const fullDetailedText = hasContent ? summary.detailedSummary.map(s => `${s.topic}.\n\n${s.content}`).join('\n\n') : '';
      return (
        ...
        <div style={{ display: 'flex', flexDirection: 'column', gap: '10px' }}>
          {summary.studyQuestions && summary.studyQuestions.length > 0 ? (
            summary.studyQuestions.map((q, i) => (
              <div key={i} style={{ display: 'flex', gap: '12px', padding: '14px 16px', background: 'var(--accent-glow)', borderRadius: '8px', border: '1px solid var(--border)' }}>
